...
$this->params['breadcrumbs'][] = ['label' => 'Plan ID '.$model->ID_PLAN, 'url'=>['plan/dashboard-sub', 'id'=>$model->ID_PLAN]];
					...
					<div class="table-responsive">
						...
						<!-- 			<td><?= $form->field($constancia, "[$i]TIPO_CONSTANCIA")->dropDownList(Constancia::getAllContanciasType(),['prompt'=>'- Seleccione -','style' => 'width: 130px;'])->label(false) ?></td> -->
									...
									<?php if (!$constancia->isNewRecord){?>
										<?= Html::a('<i class="fa fa-download"></i>', ['constanciapdf-sub', 'id'=>$constancia->ID_CONSTANCIA],  ['target' => '_blank',  'class' => 'btn btn-success btn-xs' ]) ?>
										<?= Html::a('<i class="fa fa-trash"></i>', ['constancias/delete-constancia-sub', 'id'=>$constancia->ID_CONSTANCIA], ['class' => 'btn btn-danger btn-xs',  'data' => ['confirm' => '¿Realmente quiere borrar esta constancia?',
                												  'method' => 'post',
            														],]) ?>
									<?php }else{?>
										 ...
										 <?php \yii\widgets\Pjax::begin(['id' => 'workers-ajax','timeout' => false, 'enablePushState' => false, ]); ?>
											    <?= GridView::widget([
													...
													[
														...
														return  Html::a('<i class="fa fa-check-circle-o"></i>&nbsp; Crear',['add-constancia-sub','id'=>Yii::$app->request->get('id'),'id_trab'=>$data->ID_TRABAJADOR],
