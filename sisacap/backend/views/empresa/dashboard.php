                            ...
                            <div class="small-box bg-light-blue">
                                <div class="inner">
                                    ...
                                    <i class="fa  fa-list"></i>
                                        ...
                                        <?= count(ListaPlan::findBySql("SELECT * FROM tbl_lista_plan where id_plan in (select ID_PLAN from  tbl_plan where ID_EMPRESA  = $model->ID_EMPRESA and ACTIVO  = 1) and activo =   1")->all()); 
                                        
                                        
                                        ?>
                                    </h3>
                                    <p>
                                        Reporte de constancias emitidas
                                    </p>
                                </div>
                                <div class="icon">
                                    <i class="ion ion-person-add"></i>
                                   ...
                                   <?php $i=0; foreach($model->comisionMixtaCaps as $comision):?> 
                                    <div id="tab_1_comision_<?= $comision->ID_COMISION_MIXTA?>" class="tab-pane <?= (!$i++)?'active':''?>">
                                      ...
                                      <dl class="dl-horizontal">
                                        ...
                                        <dt><i>Estatus</i></dt>
                                        <dd><span class="label label-info"><?= $comision->getStatus(); ?></span></dd>
                                    </dl> 
                                   ...
                                   <?php $i=0; foreach($plansItems as $plan):?> 
                                    <div id="tab_1_plan_<?= $plan->ID_PLAN?>" class="tab-pane <?= (!$i++)?'active':''?>">
                                      ...
                                      <dl class="dl-horizontal">
                                        ...
                                        <dt><i>Estatus</i></dt>
                                        <dd><span class="label label-info"><?= $plan->getStatus(); ?></span></dd>
                                    </dl>  
                                    <p class="text-right">
          ...
          Constancias 
   		<small>Constancias emitidas a los trabajadores</small>
            ...
            <button title="" data-toggle="tooltip" data-widget="remove" class="btn btn-default btn-xs" data-original-title="Remove"><i class="fa fa-times"></i></button>
							...
							$constanciasItems = Constancia::findBySql("SELECT * FROM tbl_constancia where id_curso in (select  ID_CURSO from tbl_curso where ID_PLAN in (select id_plan from tbl_plan where id_empresa = $model->ID_EMPRESA)  ) and activo  =  1")->all();
							...
							<?php $worker = $constancia->iDTRABAJADOR;?>
								...
								<tr>
									...
									<td><?= "Id " . $constancia->iDCURSO->ID_PLAN . " - ". $constancia->iDCURSO->iDPLAN->ALIAS;?></td>
